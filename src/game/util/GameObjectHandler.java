...
/**
 * <p>
 * The GameObjectHandeler is a class responsible for storing and receiving all the {@link GameObject GameObjects} in a scene. <br>
...
public class GameObjectHandler{
	...
	public <T extends GameObject> void addGameObject(T gameObject){
		...
		if(objects == null){
			gameObjectMap.put(gameObject.getZOrder(), objects = new CopyOnWriteArrayList<GameObject>());
		}
		...
		if(!zLevels.contains(gameObject.getZOrder())){
			zLevels.add(gameObject.getZOrder());
			zLevels.sort(null);
		}
		idHandler.addObject(gameObject);
		objectsChanged = true;*/
	}
	...
	public <T extends GameObject> void addGameObject(T gameObject, String nameID){
		...
		idHandler.addObject(gameObject, nameID);
		objectsChanged = true;*/
	}
	...
	/**
	 * This method finds all the registered {@link GameObject GameObjects} extending a certain subclass T.
	 * 
	 * @param classT The requested {@link Class}
	 * @return A ArratList of all the {@link GameObject GameObjects} extending the class T.
	 */
	public <T extends GameObject> CopyOnWriteArrayList<T> getAllGameObjectsExtending(Class<T> classT){
		//System.out.println("Called getAllGameObjectsExtending() with arguments: " + classT);
	...
	public CopyOnWriteArrayList<GameObject> getAllGameObjectsAtZLevel(int zLevel){
		//System.out.println("Called getAllGameObjectsAtZLevel() with arguments: " + zLevel);
	 ...
	 * @return
	 */
	public <T extends GameObject> CopyOnWriteArrayList<T> getAllGameObjectsAtZLevelGameObjectExtending(int zLevel, Class<T> classT){
		//System.out.println("Called getAllGameObjectsAtZLevelGameObjectExtending() with arguments: " + zLevel + " and " + classT);
		CopyOnWriteArrayList<T> returnList = new CopyOnWriteArrayList<T>();
		for(int i = 0; i < gameObjects.size(); i++){
			if(gameObjects.get(i).getZOrder() == zLevel){
