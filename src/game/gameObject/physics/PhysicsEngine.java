...
public class PhysicsEngine implements UpdateListener{
	...
	public void update(long timeMillis) {
		...
		if(gameObjectHandeler.haveObjectsChanged()){
			...
			//System.out.println("Fetching gameObjects: " + (endTime - startTime)/1000000000f);
		}
		...
		for(CopyOnWriteArrayList<Collidable> collidablesInLayer : collidables){
			for(int c1 = 0; c1 < collidablesInLayer.size(); c1++){
				for(int c2 = c1 + 1; c2 < collidablesInLayer.size(); c2++){
					if(c1 != c2){
						if(collidablesInLayer.get(c1).getBounds().intersects((Rectangle2D) collidablesInLayer.get(c2).getBounds())){
		...
		//long endTime = System.nanoTime();
		//System.out.println("Collition check: " + (endTime - startTime)/1000000000f);
		
		//System.out.println("PhysicsEngine.update() made " + checks + " collition checks");
	}
}
